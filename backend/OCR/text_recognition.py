...
def recognize_text():
	...
	#load image 
	image = cv.imread('images/IMG4.jpg')
	#copy the image 
	orig = image.copy()
	#apply east detector
	...
	if(len(stripped)>6):
		stripped = stripped[:7]		
	#turn text into lower case letters
	text_lower  = [x.lower() for x  in stripped]
	#remove some unnecessary words if exixts
	text_str =" ".join(text_lower)
	if text_str.find("series") > 0   or text_str.find("edition") > 0: 
		final = []
		#print(text_lower)
		for i , j  in enumerate(text_lower) :
			if (j=='edition' or j == 'series'):
				final.append(text_lower[i+1:])
